...
const LinkedListBlock = () => {
    ...
    return (
        <div className="p-4 md:p-10 text-cyan-300 flex flex-col items-center">
            <h1 className="text-orange-500 text-2xl md:text-4xl font-bold text-center">Singly Linked List Visualizer</h1>
            ...
            <div className="flex flex-wrap place-items-center justify-center mt-10 gap-0 md:gap-0 max-w-full">
                {list.map((node, index) => (
                    <React.Fragment key={index}>
                        <div className="bg-gray-800 p-0 md:p-0 rounded-lg mt-3 shadow-lg flex flex-row items-center w-25 sm:w-32">
                            <div className='w-[50%] px-2 h-8 border-blue-500 border-x-2 border-y-2 place-items-center rounded-md'><p className="text-sm md:text-base">{node.data}</p></div>
                            <div className='w-[50%] px-2 h-8 border-red-500 border-x-2 border-y-2 place-items-center rounded-md'><p className="text-sm md:text-base">{index === list.length - 1 ? 'Null' : 'Next'}</p></div>
                        </div>
                        {index < list.length - 1 && (
...
export default LinkedListBlock;
